...
def live_details():
    ...
    try:
        ...
        start_date_obj = datetime.strptime(input_start, "%Y-%m-%d %H:%M:%S")
    except Exception as e:
    ...
    if patients:
        print("📍 Sample patient_records ID:", patients[0].get("id"))
    if appointments:
        print("📍 Sample appointment_records IDs:", [a.get("id") for a in appointments[:5]])
    ...
    demo_names = [
        "Dr. Adams", "Dr. Lane", "Dr. Brooks", "Dr. Kim",
        "FNP Jackson", "FNP Morales", "PA Rivers", "PA Chen"
    ]
    unique_providers = merged_df['Provider Name'].unique()
    provider_map = {real: demo_names[i % len(demo_names)] for i, real in enumerate(unique_providers)}
    merged_df['Provider Name'] = merged_df['Provider Name'].map(provider_map)

    merged_df['Patient ID'] = merged_df['patient_id_x']
    department_map = {
        "6": "Wellness Center",
        "18": "Community Health Services",
        "22": "Riverside Medical",
        "27": "Downtown Family Practice",
        "28": "Greenway Outreach",
        "31": "Northview Clinic",
        "33": "Downtown Telehealth",
        "34": "Bayou Family Care",
        "35": "Midtown Pediatrics",
        "37": "Northview Telehealth",
        "38": "Lakeside Health Hub",
        "39": "Greenway Teleclinic",
        "40": "Terrace Medical Group",
        "41": "Hillside School Wellness",
        "42": "Terrace Televisit",
        "43": "Greenway Virtual",
        "44": "Bayou Virtual",
        "45": "Midtown Telehealth",
        "46": "Lakeside Virtual",
        "47": "Hillside Televisit"
    }
    ...
    return jsonify({
        "records": merged_df[columns].to_dict(orient='records')
    })
