...
useEffect(() => {
    ...
    if (!startTime.includes('null') && !endTime.includes('null')) {
      axios.get(`${baseUrl}/live-details`, {
        ...
        .then(res => {
          ...
          const filteredData = Array.isArray(res.data) ? res.data : (res.data.records || []);
  ...
  return (
    <Router>
      <Routes>
        <Route path="/admin" element={<Admin setLabel={setLabel} setLogoUrl={setLogoUrl} />} />
        ...
        <Route path="/" element={
          <ProtectedRoute>
            {loadingDate || !selectedDate ? (
              <>
                <div style={{ backgroundColor: '#ffcc00', padding: '10px', textAlign: 'center', fontWeight: 'bold', color: '#333' }}>
                  DEMO MODE – This is simulated data only
                </div>
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 30 }}>
                <img src={logoUrl} alt="Logo" style={{ height: 140, marginRight: 20 }} />
                <h1 style={{ fontSize: '1.8rem', margin: 0 }}>{label}</h1>
              </div>
              {/* Date Mode & AI Show Rate Mode Section - Improved alignment */}
              <div style={{ display: 'flex', gap: '60px', alignItems: 'flex-start', marginBottom: 20 }}>
